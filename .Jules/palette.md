...
## 2024-08-09 - Escape key to close progressive disclosure
**Learning:** Progressive disclosure forms (like the "Create Lobby" section) lacked an accessible keyboard shortcut to cancel or close them. Users expecting to press "Escape" to close the view were forced to locate and click the Cancel button instead.
**Action:** When designing progressive disclosure forms or modals, always pair them with an explicit 'Cancel' or 'Close' button. Map the standard 'Escape' key to trigger this cancellation, and ensure the shortcut is documented in the button's `title` and `aria-keyshortcuts` attributes to improve discoverability and keyboard accessibility.
