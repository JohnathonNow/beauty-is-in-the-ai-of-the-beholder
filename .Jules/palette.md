...
## 2026-09-03 - Progressive Disclosure Focus Management
**Learning:** When progressively disclosing new forms (like opening the Lobby Creation section) or navigating between major views (like logging in and moving to Lobby Selection), screen reader and keyboard users can lose their place if focus is not explicitly managed. Relying on default focus order often leaves users at the top of the document or on elements that are no longer relevant or visible.
**Action:** Always programmatically manage focus during view transitions or progressive disclosure. Focus the first logical input or a newly revealed heading (`tabindex="-1"`) when moving forward, and explicitly return focus to the triggering element (like the "Create Lobby" button) when cancelling or closing a section.
