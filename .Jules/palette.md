...
## 2026-08-22 - [Escape key for Lobby Creation]
**Learning:** Progressive disclosure forms or stateful sections (like the lobby creation view) require an explicit 'Cancel' or 'Close' action. While a button is helpful, power users expect keyboard shortcuts like 'Escape' to dismiss modals and sub-forms. Without this, users might feel stuck if they prefer keyboard navigation.
**Action:** When designing progressive disclosure forms, stateful sections, or modals (like the lobby creation view), always pair them with an explicit 'Cancel' or 'Close' button. Map the standard 'Escape' key to trigger this cancellation, and ensure the shortcut is documented in the button's `title` and `aria-keyshortcuts` attributes to improve discoverability and keyboard accessibility.
