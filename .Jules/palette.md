...
## 2024-02-23 - Focus Management on SPA View Transitions
**Learning:** In SPAs or heavily JS-driven vanilla applications where views are swapped out using `display: none` and `display: block`, simply revealing the new view does not move the screen reader or keyboard focus. If a user triggers a transition (like clicking "Join"), their focus remains on the old button or moves to the top of the body, which is confusing and breaks logical tabbing order.
**Action:** When programmatically changing the active "view" container in a vanilla JS application, add `tabindex="-1"` to the primary heading of the newly visible container and explicitly call `.focus()` on it immediately after making the container visible.
