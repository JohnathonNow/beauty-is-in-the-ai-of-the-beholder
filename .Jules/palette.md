...
## 2026-10-24 - Focus Management on View Transitions
**Learning:** In single-page applications without standard page reloads, transitioning between major views (like Lobby to Game) or progressively disclosing forms (like Create Lobby) leaves screen reader users and keyboard navigators stranded without context, as focus remains on the triggering element or defaults to the document body.
**Action:** When progressively disclosing new forms (like opening a creation section) or navigating between major views, always programmatically manage focus. Focus the first logical input or a newly revealed heading (using `tabindex="-1"`) when moving forward, and return focus to the triggering element when cancelling or closing to maintain keyboard/screen reader accessibility.
