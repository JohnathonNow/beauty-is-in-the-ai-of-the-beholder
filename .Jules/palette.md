...
## 2024-11-20 - Focus Management During View Transitions
**Learning:** In single-page applications or UI's with major view swaps (like moving from a Login screen to a Lobby, or Lobby to Game), simply hiding and showing containers using `display: none`/`block` loses the user's focus. This forces keyboard users and screen reader users to tab from the beginning of the document again or leaves them confused about what just appeared.
**Action:** When transitioning between major views or progressively disclosing significant new sections, always programmatically manage focus. Add `tabindex="-1"` and `role="region"`/`aria-label` to the new container or its main heading, and call `.focus()` on it during the transition via JavaScript.
