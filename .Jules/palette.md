...
**Action:** Expose tool shortcuts (Pencil, Eraser, etc.) via visual tooltips and `aria-keyshortcuts`, and safely listen to global key events checking for tag name and modifier keys before performing an action in drawing apps.## 2026-09-12 - Dynamic Descriptions for Dropdowns
**Learning:** Dropdowns with opaque or generic labels (like "AI", "Evolution") leave users unsure of what the selection does. Adding a dynamic description field below the selector that updates on change provides immediate, contextual guidance. Screen readers require this container to have `role="status"` and `aria-live="polite"` so they are notified when the description changes.
**Action:** Always implement dynamic descriptive text for complex dropdown selections and ensure the container uses `aria-live` regions to announce the context change to assistive technologies.
